MCABiPlot <- function() {
  ...
  formatted[grepl('sus', rownames(formatted)),]$color <- rgb(27,158,119,0.8)
  formatted[grepl('sns', rownames(formatted)),]$symbol <- 'cross-dot'
  formatted[grepl('sns', rownames(formatted)),]$color <- rgb(217,95,2,0.8)
  formatted[grepl('^ACP', rownames(formatted)),]$symbol <- 'diamond-x'
  formatted[grepl('^ACP', rownames(formatted)),]$color <- rgb(117,112,179,0.8)
